...
test_that('No drought produces a single censored event', {
    ...
    expected <- data.table::data.table(id=1, groupid=1, tstart=seq(0, (nyear-1)*12, 12),
                           tstop=seq(12, nyear*12, 12), drought=0, Tg=Tgyr)

    expect_is(ev, 'data.table')
...
test_that('One drought produces the expected result', {
    ...
    expected2 <- data.table::data.table(id=c(101, rep(102, nyear)),
                            groupid=2,
    ...
    expected3 <- data.table::data.table(id=c(201, 201, rep(202, nyear-2)),
                            groupid=3,
    ...
    expected4 <- data.table::data.table(id=c(rep(301,3), rep(302, 5)),
                            groupid=4,
...
test_that('Multiple droughts produce the expected result', {
    ...
    expected <- data.table::data.table(id=c(401,401,402, rep(403, 5)),
                           groupid=5,
    ...
    expected2 <- data.table::data.table(id=rep(c(501, 502, 503), c(2,4,5)),
                            groupid=6,
